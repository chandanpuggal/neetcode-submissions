...
bool comparatorFunc(const pair<int, int> &a, const pair<int, int> &b) {
    return a.second > b.second;
}
    ...
    */
    ...
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        for (auto it = numsMap.begin(); it != numsMap.end(); it++) {
            countVector.push_back(make_pair(it->first, it->second));
        }

        sort(countVector.begin(), countVector.end(), comparatorFunc);
